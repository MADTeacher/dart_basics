...
void main() async {
  ...
  await db.transaction((trx) async {
    ...
    try{
    // Так как группа  с этим именем уже существует, 
    // то будет выброшено исключение и groupId не будет изменен
     groupId = await trx.insert(
      'student_group',
      {'name': '4316'},
    );
    } catch (e) {
    ...
    try{ // будет выполнено без исключений
    // вставляем нового студента
    await trx.insert('student', {
      'full_name': 'Кузнецов Сергей Сергеевич',
      'group_id': groupId,
      'email': 'kuznetsov@ss.com',
    });
    // вставляем нового студента
    await trx.insert('student', {
