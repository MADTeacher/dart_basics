...
void main() async {
  ...
  // Извлекаем всех студентов с фамилией, начинающейся на 'М', из таблицы student
  // посредством sql-запроса
  var query = 'SELECT id, full_name FROM student ';
  query += 'WHERE full_name LIKE ?';
  final rawQuery = await db.rawQuery(query, ['М%']);
  ...
  final students =
      await db.query('student', where: 'full_name LIKE ?', whereArgs: ['М%']);
  print(students);
  print('--------------------------------');
