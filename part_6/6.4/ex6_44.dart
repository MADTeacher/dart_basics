...
void main(List<String> arguments) async {
  ...
  SendPort? sendPort;
  SendPort? secondIsolateSendPort;
  var count = 0;
  ...
  stdin.transform(utf8.decoder).listen((data) async {
    ...
    if (input == 'kill') {
      ...
      var stlitData = input.split(' ');
      if (stlitData.length == 2) {
        var direct = int.tryParse(stlitData[0]);
        if (direct != null) {
          direct == 1
              ? sendPort?.send(stlitData[1])
              : secondIsolateSendPort?.send(stlitData[1]);
        } else {
          print('Invalid input');
        }
      } else {
